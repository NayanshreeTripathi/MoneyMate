...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  async({step}) => {
    // Fetch all recurring transactions that are due to be processed
    const recurringTramsactions =  await step.run(
    ...
    if(recurringTramsactions.length > 0){
      const events = recurringTramsactions.map((transaction)=>({
        name:"transaction.recurring.process",
        data:{transactionId: transaction.id , userId: transaction.userId},
      }));
      // Send events to Inngest to processed
      await inngest.send(events);
    }
    return {triggered: recurringTramsactions.length};
  }
)
