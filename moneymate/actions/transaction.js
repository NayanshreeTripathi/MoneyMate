...
export async function createTransaction(data) {
    try {
        const {userTd} = await auth();
        if (!userTd) throw new Error("Unauthorized");
        const user = await db.user.findUnique({
            where: { clerkUserId: userTd },
        });
        if (!user) {
        ...
        const transaction = await db.$transaction(async (tx) => {
            const newTransacrion = await tx.transaction.create({
                data: {
                    ...data,
                    userId: user.id,
